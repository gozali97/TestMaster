...
class TestGenerator {
     ...
     */
    async generateWebsiteTests(websiteMap) {
        ...
        const formsPages = websiteMap.pages.filter((p) => p.elements.some((e) => e.type === 'form'));
     ...
     */
    async generateE2ETests(appMap) {
        // Generate combined web + API tests
        // Example: User registration on web triggers API call
        if (appMap.website?.userFlows) {
            const registerFlow = appMap.website.userFlows.find(f => f.name.toLowerCase().includes('register'));
            if (registerFlow) {
     ...
     */
    async generateUserFlowTest(flow) {
     ...
     */
    async generateFormTests(page) {
        const tests = [];
        const forms = page.elements.filter((e) => e.type === 'form');
        for (const form of forms) {
            // Find inputs in this form
            const inputs = page.elements.filter((e) => e.type === 'input' && e.locator.includes(form.locator));
            if (inputs.length > 0) {
     ...
     */
    generateInteractionTests(websiteMap) {
        const tests = [];
     ...
     */
    async generateAPIEndpointTest(endpoint, apiMap) {
    ...
    generateValidValue(input) {
        ...
        if (locator.includes('password'))
            return 'Test@123456';
        if (locator.includes('name'))
            return 'Test User';
        if (locator.includes('phone'))
            return '+1234567890';
        if (locator.includes('address'))
            return '123 Test Street';
        if (locator.includes('city'))
            return 'Test City';
        if (locator.includes('zip'))
            return '12345';
        return 'Test Value';
    }
    /**
