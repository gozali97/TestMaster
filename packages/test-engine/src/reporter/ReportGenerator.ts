...
export class ReportGenerator {
  ...
  async generate(data: ReportData): Promise<Report> {
    ...
    return {
      summary,
      details: {
        tests: data.executionResults.passed.concat(
          data.executionResults.failed,
          data.executionResults.healed
        ),
   ...
   */
  private generateSummary(data: ReportData) {
    ...
    const coverage = Math.min(
      ((pagesDiscovered + apisDiscovered) / Math.max(total, 1)) * 100,
      100
    );
   ...
   */
  private generateHTML(data: ReportData, summary: any): string {
...
<body>
   ...
   */
  private generateJSON(data: ReportData, summary: any): string {
