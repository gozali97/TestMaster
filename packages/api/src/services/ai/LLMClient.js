...
const logger = new logger_1.Logger('LLMClient');
class LLMClient {
    static instance;
    openai = null;
    enabled;
     ...
     */
    async cacheResponse(cacheKey, response, ttl = 86400) {
        try {
    ...
    async getEmbeddings(text) {
        ...
        if (cached && cached.expires > Date.now()) {
            ...
            return cached.response; // embeddings stored as response
        }
        // Rate limiting
        await this.checkRateLimit();
        try {
...
exports.default = LLMClient.getInstance();
